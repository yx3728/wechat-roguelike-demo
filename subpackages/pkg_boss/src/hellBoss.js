 ...
 *
 * 三招的走廊与位移预算都验算过（玩家为手指直接拖拽，按可控 700px/s、读预警 400ms）：
 *   · 锁刑：朝下的弹幕墙，缺口在玩家那一排恒为 59px（已发货档位 45–89px 的紧端）
 *   · 缺口左右摆动：一阶段 ±110px / 间隔 620ms，二阶段 ±140px / 间隔 480ms
 *   · 无间 3×4 两格安全，**至少一个安全格与玩家当前格相邻** → 最坏位移 248px
 * 改这些数之前先看 docs/hell-map.md 第五节。
 * ----------------------------------------------------------------------------
 */
...
const WAVE_GAP_PX = 59;           // 缺口在玩家那一排的宽度（玩家宽 32，已发货档位 45–89 的紧端）
